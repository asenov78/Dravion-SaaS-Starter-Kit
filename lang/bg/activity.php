<?php

return [
    'title'              => 'Дневник на активността',
    'subtitle'           => 'История на системната и потребителска активност',
    'empty'              => 'Все още няма записана активност.',
    'search_placeholder' => 'Търси в описания…',
    'filter_event'       => 'Всички типове',
    'filter_user'        => 'Всички потребители',
    'filter_date_from'   => 'От дата',
    'filter_date_to'     => 'До дата',
    'export_csv'         => 'Експортирай CSV',
    'event'       => 'Събитие',
    'description' => 'Описание',
    'user'        => 'Потребител',
    'subject'     => 'Обект',
    'when'        => 'Кога',
    'system'      => 'Система',
    'events'      => [
        'created'          => 'Създаден',
        'updated'          => 'Обновен',
        'deleted'          => 'Изтрит',
        'suspended'        => 'Спрян',
        'activated'        => 'Активиран',
        'restored'         => 'Възстановен',
        'removed'          => 'Премахнат',
        'login'            => 'Вход',
        'logout'           => 'Изход',
        'failed'           => 'Неуспешен',
        'category_created' => 'Категория създадена',
        'category_updated' => 'Категория обновена',
        'category_deleted' => 'Категория изтрита',
        'field_created'    => 'Поле създадено',
        'field_updated'    => 'Поле обновено',
        'field_deleted'    => 'Поле изтрито',
    ],
    'log'         => [
        'user_created'        => 'Създаден потребител :name (:email)',
        'user_updated'        => 'Обновен потребител :name (:email)',
        'user_suspended'      => 'Спрян потребител :name (:email)',
        'user_activated'      => 'Активиран потребител :name (:email)',
        'user_deleted'        => 'Изтрит потребител :name (:email)',
        'user_restored'       => 'Възстановен потребител :name (:email)',
        'role_created'        => "Ролята ':role' е създадена",
        'role_renamed'        => "Ролята ':old' е преименувана на ':new'",
        'role_deleted'        => "Ролята ':role' е изтрита",
        'permissions_updated' => 'Матрицата с права е обновена от :name',
        'settings_updated'    => 'Системните настройки са обновени от :name',
        'profile_updated'     => 'Профилът на :name (:email) е обновен',
        'user_logged_in'      => ':name влезе в системата',
        'user_logged_out'     => ':name излезе от системата',
        'login_failed'        => 'Неуспешен опит за вход с :email',
        'login_suspended'     => 'Опит за вход в спряна акаунт: :email',
        'license_activated'   => 'Лицензът е активиран за домейн :domain',
        'license_removed'     => 'Лицензът е премахнат за домейн :domain',
    ],
];
