    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <div class="hidden md:flex items-center gap-3">
                @auth
                    @if(auth()->user()->hasAnyRole(['admin', 'manager', 'editor']))
                        <a href="{{ route('admin.dashboard') }}"
                           class="inline-flex items-center px-4 py-2 rounded-lg bg-brand-500 text-white text-sm font-semibold hover:bg-brand-600 transition-colors">
                            {{ __('nav.dashboard') }}
                        </a>
                    @else
                ...
                <button type="button" id="public-theme-toggle"
                        onclick="(function(){var d=document.documentElement.classList.contains('dark');if(d){localStorage.setItem('theme','light');document.documentElement.classList.remove('dark');}else{localStorage.setItem('theme','dark');document.documentElement.classList.add('dark');}})()"
        ...
        <div class="px-4 py-3 space-y-1">
            ...
            <div class="pt-2 border-t border-gray-200 dark:border-gray-700 space-y-1">
                @auth
                    @if(auth()->user()->hasAnyRole(['admin', 'manager', 'editor']))
                        <a href="{{ route('admin.dashboard') }}"
                           class="block px-3 py-2 rounded-lg text-sm font-semibold text-brand-500 hover:bg-gray-100 dark:hover:bg-gray-800">
                            {{ __('nav.dashboard') }}
                        </a>
                    @else
                        <a href="{{ route('dashboard') }}"
