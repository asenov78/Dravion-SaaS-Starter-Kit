...
<div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $u->country ?: '—' }}</p>
                    </div>
                    <div>
                        <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">City/State</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $u->city_state ?: '—' }}</p>
                    </div>
                    <div>
                        <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Postal Code</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $u->postal_code ?: '—' }}</p>
                    </div>
                    <div>
                        <p class="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">TAX ID</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $u->tax_id ?: '—' }}</p>
                    </div>
                </div>
            </div>
    ...
    <x-ui.modal x-data="{ open: false }" @open-profile-address-modal.window="open = true" :isOpen="false" class="max-w-[700px]">
        <div class="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
            ...
            <form method="POST" action="{{ route('admin.ui.profile.update') }}" class="flex flex-col">
                @csrf
                @method('PUT')
                {{-- carry over identity fields so update validation passes --}}
                <input type="hidden" name="name" value="{{ $u->name }}" />
                <input type="hidden" name="email" value="{{ $u->email }}" />
                <div class="px-2 overflow-y-auto custom-scrollbar">
                    <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                        @foreach (['country' => 'Country', 'city_state' => 'City/State', 'postal_code' => 'Postal Code', 'tax_id' => 'TAX ID'] as $field => $label)
                            <div>
