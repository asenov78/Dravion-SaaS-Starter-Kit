...
<head>
    ...
    <title>{{ __('auth.title_forgot') }} — Dravion</title>
...
<div style="width:100%; max-width:360px; padding:0 16px; position:relative; z-index:1;">
    <div style="text-align:center; margin-bottom:32px;">
        ...
        <h1 style="color:#e2e2e9; font-size:18px; font-weight:600; margin:0 0 4px;">{{ __('auth.title_forgot') }}</h1>
        <p style="color:#6b6b7b; font-size:13px; margin:0;">{{ __('auth.subtitle_forgot') }}</p>
    </div>
    ...
    <x-ui.card>
        ...
        <form method="POST" action="{{ route('password.email') }}">
            ...
            <div style="display:flex; flex-direction:column; gap:14px;">
                ...
                <x-ui.button type="submit" style="width:100%;">{{ __('auth.send_link') }}</x-ui.button>
            </div>
        </form>
    </x-ui.card>
    ...
    <p style="text-align:center; margin-top:16px; color:#6b6b7b; font-size:12px;">
        <a href="{{ route('login') }}" style="color:#5e6ad2; text-decoration:none;"
            onmouseover="this.style.color='#7b84e0'" onmouseout="this.style.color='#5e6ad2'">{{ __('auth.back_to_login') }}</a>
    </p>
</div>
