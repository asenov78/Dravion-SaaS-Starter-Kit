...
{{-- ══════════════════ HERO ══════════════════ --}}
<section class="relative overflow-hidden hero-gradient py-24 sm:py-32">
    {{-- Grid bg --}}
    <div class="absolute inset-0 pointer-events-none" aria-hidden="true">
        <svg class="absolute w-full h-full opacity-[0.03] dark:opacity-[0.06]" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        {{-- Badge --}}
        <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-200 dark:border-brand-500/20 mb-8">
        ...
        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-gray-900 dark:text-white tracking-tight leading-tight mb-6">
            Пълноценен SaaS Starter Kit<br>
            <span class="text-brand-500">готов за продукция</span>
        </h1>
        <p class="text-lg sm:text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mb-10">
            Спестете месеци разработка. Ролева система, лицензиране, само-обновяване,
            in-app нотификации, CMS и десетки готови компоненти — всичко в едно.
        </p>

        <div class="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            @guest
            <a href="{{ route('register') }}"
               class="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl bg-brand-500 text-white font-semibold text-base hover:bg-brand-600 transition-all shadow-lg shadow-brand-500/25 hover:shadow-brand-500/40">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                Стартирай безплатно
            </a>
            <a href="{{ route('login') }}"
               class="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-semibold text-base hover:bg-gray-50 dark:hover:bg-gray-800 transition-all">
                Влез в системата
            </a>
            @else
            <a href="{{ auth()->user()->hasAnyRole(['admin','manager','editor']) ? route('admin.dashboard') : route('dashboard') }}"
               class="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl bg-brand-500 text-white font-semibold text-base hover:bg-brand-600 transition-all shadow-lg shadow-brand-500/25">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 max-w-3xl mx-auto">
            ...
            <div class="text-center">
                <div class="text-3xl font-extrabold text-brand-500 mb-1">{{ $num }}</div>
